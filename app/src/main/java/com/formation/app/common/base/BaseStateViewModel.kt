package com.formation.app.common.base
...
abstract class BaseStateViewModel<S> constructor(initialState: S) : ViewModel() {
    ...
    val stateFlow : StateFlow<S> = state
    ...
    protected fun CoroutineScope.launchSetState(reducer: S.() -> S) {
        launch { this@BaseStateViewModel.setState(reducer) }
    }
    ...
    protected fun CoroutineScope.withState(block: (S) -> Unit) {
        launch { this@BaseStateViewModel.withState(block) }
    }
}